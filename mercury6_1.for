...
c Local
      integer j
      real*8 adotovera(nbig-1),edotovere(nbig-1), idotoveri(nbig-1)
      real*8 peri, drde,drfdotde,drdotde,r, msum, rdot, rfdot
      real*8 periq, e, inc, pomega, node, Manom, f, temp, semi
      real*8 tau, factor, afs(4), v2, ais(4), gm, af, ai,taue,ei,ef
c
c------------------------------------------------------------------------------
c
c     Fig. 3
...
c Don't migrate/damp central body or test particles
c Get orbital elements
               ai = ais(j-1)
               af = afs(j-1)
               msum = m(1) + m(j)
               call mco_x2el (msum,x(1,j),x(2,j),x(3,j),v(1,j),
     $              v(2,j),v(3,j),
     $              periq, e, inc, pomega, node, Manom)
c      if (time.ge.335500000) print 100,msum,x(1,j),x(2,j),x(3,j),v(1,j),
c     $              v(2,j),v(3,j),
c     $              periq, e, inc, pomega, node, Manom
 100  format (f100.80)
              semi = periq/(1.d0-e)
              adotovera(j-1) = (af-ai)/tau*exp(-time/tau)/semi  
              edotovere(j-1) = (ef-ei)/taue*exp(-time/taue)/e
c Turn off ecc migration if e drops below current Neptune e 
c	      if (e.lt.0.011214269) then
c                 edotovere(j-1) = 0.d0
c              endif
              if (edotovere(j-1).ne.0) then
                 if (e.eq.0) then
                    f = Manom
                 else
                    temp = (periq*(1.d0 + e)/(x(1,j)**2.d0+x(2,j)**2.d0
     $                +x(3,j)**2.d0)**(1.d0/2) - 1.d0) /e
                 ...
                 rdot = sqrt(msum)*semi**(-1.d0/2)
     $              *e*sin(f)
     $                *(1.d0-e**2.d0)**(-1.d0/2)
                 drdotde = rdot/(e*(1.d0-e**2.d0))
                 rfdot = sqrt(msum)*semi**(-1.d0/2)
     $                *(1.d0+e*cos(f))
     $                *(1.d0-e**2.d0)**(-1.d0/2)
                 drfdotde = rfdot*(e+cos(f))/
     $              ((1.d0-e**2.d0)*(1.d0+e*cos(f)))
                 peri = mod(pomega - node + TWOPI, TWOPI)
                 drde = -2.d0*e*r/(1.d0-e**2.d0)-r**2.d0*cos(f)
     $              /(semi*(1.d0-e**2.d0))

               end if

               a(1,j) =  -(v(1,j)+3.d0*idotoveri(j-1)*inc*x(3,j))
     $              /2.d0*adotovera(j-1)
               a(2,j) =  -(v(2,j)-3.d0*idotoveri(j-1)*inc*x(3,j))
     $              /2.d0*adotovera(j-1)
               a(3,j) =  -(v(3,j)+3.d0*idotoveri(j-1)*inc*x(3,j))
     $              /2.d0*adotovera(j-1)
               vu(1,j) = x(1,j)*adotovera(j-1)
               vu(2,j) = x(2,j)*adotovera(j-1)
               vu(3,j) = x(3,j)*adotovera(j-1)
     ...
     $              *sin(peri+f))
     $              - sin(node)*
     $              (drdotde*cos(inc)*sin(peri+f)
     $              +drfdotde*cos(inc)*cos(peri+f)
     $              -drde*x(3,j)/r*idotoveri(j-1)*inc)
     $              )
     $              *edotovere(j-1)*e
                    a(2,j) = a(2,j)+ (
     $              sin(node)*(drdotde*cos(peri+f)-drfdotde*sin(peri+f))
     $              + cos(node)*
     $              (drdotde*cos(inc)*sin(peri+f)
     $              +drfdotde*cos(inc)*cos(peri+f)
     $              -drde*x(3,j)/r*idotoveri(j-1)*inc)
     ...
     $              *edotovere(j-1)*e
                    a(3,j) = a(3,j) 
     $              + (drdotde*cos(inc)*sin(peri+f)+drfdotde*sin(inc)
     $              *cos(peri+f)
     $              +drde*cos(inc)*sin(peri+f)*idotoveri(j-1)*inc)
     $              *edotovere(j-1)*e
                    vu(1,j) = vu(1,j) + (r/(semi*(1.d0-e**2))-(1.d0
     $                   +e**2)/(1.d0-e**2))*x(1,j)*edotovere(j-1)
                    vu(2,j) = vu(2,j) + (r/(semi*(1.d0-e**2))-(1.d0
     $                   +e**2)/(1.d0-e**2))*x(2,j)*edotovere(j-1)
                    vu(3,j) = vu(3,j) + (r/(semi*(1.d0-e**2))-(1.d0
     $                   +e**2)/(1.d0-e**2))*x(3,j)*edotovere(j-1)
               end if

               if (idotoveri(j-1).ne.0) then
                   a(1,j) = a(1,j) + sin(node)*(v(3,j)+x(3,j)
     $                 *idotoveri(j-1))
     $              *idotoveri(j-1)*inc
                   a(2,j) = a(2,j) +  - cos(node)*(v(3,j)+x(3,j)
     $              *idotoveri(j-1))
     $              *idotoveri(j-1)*inc
                    a(3,j) = a(3,j)
     $               +(
     $              (-sin(node)*v(1,j)+cos(node)*v(2,j))+
     $              idotoveri(j-1)*(-sin(node)*x(1,j)+cos(node)*x(2,j))
     $              )
     $              *idotoveri(j-1)*inc
                    vu(1,j) = vu(1,j)+x(3,j)*sin(node)*inc
     $                   *idotoveri(j-1)
                    vu(2,j) = vu(2,j)-x(3,j)*cos(node)*inc
     $                   *idotoveri(j-1)
                    vu(3,j) = vu(3,j)+(-x(1,j)*sin(node)+x(2,j)*
     $                   cos(node))*inc*idotoveri(j-1)
               end if
            end if
        end do
...
c Local
      ...
      real*8 xsave(3,NBIG),vsave(3,NBIG)
...
c      print 101,xh(1,5),xh(2,5),xh(3,5),vh(1,5),vh(2,5),vh(3,5)
c      print 101,x(1,5),x(2,5),x(3,5),v(1,5),v(2,5),v(3,5)
...
c      print 101,x(1,5),x(2,5),x(3,5),v(1,5),v(2,5),v(3,5)
c      pause
c coord bcoord mco_h2dh,mco_dh2h
c
c------------------------------------------------------------------------------
c
c  MAIN  LOOP  STARTS  HERE
...
c Make sure the integration is heading in the right direction
 150  continue
      if (time.lt.0) print *,'flag g'
      if (time.lt.0) print 101,v(3,5),x(3,5)      
     $     ,v(2,5),x(2,5),v(1,5),x(1,5)
      if (time.lt.0) print 101,vh(3,5),xh(3,5)      
     $     ,vh(2,5),xh(2,5),vh(1,5),xh(1,5)
      tmp0 = tstop - time
      if (opflag.eq.-1) tmp0 = tstart - time
      h0 = sign (h0, tmp0)
c
c Save the current heliocentric coordinates and velocities
 ...
 101  format (f100.80)
     ...
     %  mem,lmem)
      time = time + h0
      if (time.lt.0) print *,'flag b'
      if (time.lt.0) print 101,v(3,5),x(3,5)      
     $     ,v(2,5),x(2,5),v(1,5),x(1,5)
      if (time.lt.0) print 101,vh(3,5),xh(3,5)      
     $     ,vh(2,5),xh(2,5),vh(1,5),xh(1,5)
      if (time.lt.0) print 101,v(3,4),x(3,4)      
     $     ,v(2,4),x(2,4),v(1,4),x(1,4)
      if (time.lt.0) print 101,vh(3,4),xh(3,4)      
     $     ,vh(2,4),xh(2,4),vh(1,4),xh(1,4)
      if (time.lt.0) print 101,v(3,3),x(3,3)      
     $     ,v(2,3),x(2,3),v(1,3),x(1,3)
      if (time.lt.0) print 101,vh(3,3),xh(3,3)      
     $     ,vh(2,3),xh(2,3),vh(1,3),xh(1,3)
      if (time.lt.0) print 101,v(3,2),x(3,2)      
     $     ,v(2,2),x(2,2),v(1,2),x(1,2)
      if (time.lt.0) print 101,vh(3,2),xh(3,2)      
     $     ,vh(2,2),xh(2,2),vh(1,2),xh(1,2)
c
c------------------------------------------------------------------------------
c
c  CLOSE  ENCOUNTERS
...
c Remove lost objects, reset flags and recompute Hill and physical radii
     ...
     %      opt)
          ...
          call coord (time,jcen,nbod,nbig,h0,m,xh,vh,x,v,ngf,ngflag,opt)
      if (time.lt.0) print *,'flag n'
      if (time.lt.0) print 101,v(3,5),x(3,5)      
     $     ,v(2,5),x(2,5),v(1,5),x(1,5)
      if (time.lt.0) print 101,vh(3,5),xh(3,5)      
     $     ,vh(2,5),xh(2,5),vh(1,5),xh(1,5)
        end if
      if (time.lt.0) print *,'flag f'
      if (time.lt.0) print 101,v(3,5),x(3,5)      
     $     ,v(2,5),x(2,5),v(1,5),x(1,5)
      if (time.lt.0) print 101,vh(3,5),xh(3,5)      
     $     ,vh(2,5),xh(2,5),vh(1,5),xh(1,5)
...
c Convert to heliocentric coords and write a progress report to the log file
     ...
     $     ,v(2,2),x(2,2),v(1,2),x(1,2)
      if (time.lt.0) print 101,vh(3,2),xh(3,2)      
     $     ,vh(2,2),xh(2,2),vh(1,2),xh(1,2)
      if (time.lt.0) print 101,v(3,1),x(3,1)      
     $     ,v(2,1),x(2,1),v(1,1),x(1,1)
      if (time.lt.0) print 101,vh(3,1),xh(3,1)      
     $     ,vh(2,1),xh(2,1),vh(1,1),xh(1,1)
      if (time.lt.0) print 101,m(1),m(2),m(3)
     $     ,m(4),m(5)
      if (time.lt.0) print *,'flag p 1'
      if (time.lt.0) print 101,v(3,5),x(3,5)      
     $     ,v(2,5),x(2,5),v(1,5),x(1,5)
      if (time.lt.0) print 101,vh(3,5),xh(3,5)      
     $     ,vh(2,5),xh(2,5),vh(1,5),xh(1,5)
      if (time.lt.0) print *,ngf,ngflag,
     $     opt,h0,time,jcen,nbod,nbig
      if (time.lt.0) print 101,m(1),m(2),m(3),m(4),m(5)
      if (time.lt.0) print 101,v(3,4),x(3,4)      
     $     ,v(2,4),x(2,4),v(1,4),x(1,4)
      if (time.lt.0) print 101,vh(3,4),xh(3,4)      
     $     ,vh(2,4),xh(2,4),vh(1,4),xh(1,4)

      if (time.lt.0) print 101,v(3,3)
     $     ,x(3,3)      
     $     ,v(2,3),x(2,3),v(1,3),x(1,3)
      if (time.lt.0) print 101,vh(3,3),xh(3,3)      
     $     ,vh(2,3),xh(2,3),vh(1,3),xh(1,3)
     ...
     $     ,vh(2,1),xh(2,1),vh(1,1),xh(1,1)
c           if (time.lt.0) call
c     $     bcoord(time,jcen,nbod,nbig,h0,m,x,v,xh,vh,ngf,ngflag,opt)
c           if (time.lt.0)
c     $     call coord (time,jcen,nbod,nbig,h0,m,xh,vh,x,v,ngf,ngflag,
c     %        opt)
      if (time.lt.0) print *,'flag p 2'
      if (time.lt.0) print 101,v(3,5),x(3,5)      
     $     ,v(2,5),x(2,5),v(1,5),x(1,5)
      if (time.lt.0) print 101,vh(3,5),xh(3,5)      
     $     ,vh(2,5),xh(2,5),vh(1,5),xh(1,5)
            if (time.lt.0) print *,ngf,ngflag,opt,h0,time,jcen,nbod,nbig
      if (time.lt.0) print 101,m(1),m(2),m(3),m(4),m(5)
      if (time.lt.0) print 101,v(3,4),x(3,4)      
     $     ,v(2,4),x(2,4),v(1,4),x(1,4)
      if (time.lt.0) print 101,vh(3,4),xh(3,4)      
...
c Remove ejected objects, reset flags, calculate new Hill and physical radii
     ...
     %        ngflag,opt)
      if (time.lt.0) print *,'flag v'
      if (time.lt.0) print 101,v(3,5),x(3,5)      
     $     ,v(2,5),x(2,5),v(1,5),x(1,5)
      if (time.lt.0) print 101,vh(3,5),xh(3,5)      
     $     ,vh(2,5),xh(2,5),vh(1,5),xh(1,5)
          else
      if (time.lt.0) print *,'flag u'
      if (time.lt.0) print 101,v(3,5),x(3,5)      
     $     ,v(2,5),x(2,5),v(1,5),x(1,5)
      if (time.lt.0) print 101,vh(3,5),xh(3,5)      
     $     ,vh(2,5),xh(2,5),vh(1,5),xh(1,5)
     ...
     $     ,v(2,2),x(2,2),v(1,2),x(1,2)
      if (time.lt.0) print 101,vh(3,2),xh(3,2)      
     $     ,vh(2,2),xh(2,2),vh(1,2),xh(1,2)
       if (time.lt.0) print 101,m(2)/m(1)
     $     ,m(3)/m(1),m(4)/m(1),m(5)/m(1)
      if (time.lt.0) print *,'flag pre-uu'
      if (time.lt.0) print 101,v(3,5),x(3,5)      
     $     ,v(2,5),x(2,5),v(1,5),x(1,5)
      if (time.lt.0) print 101,vh(3,5),xh(3,5)      
     $     ,vh(2,5),xh(2,5),vh(1,5),xh(1,5)
     ...
     %        opt)
      ...
      if (time.lt.0) print *,'flag uu'
      if (time.lt.0) print 101,v(3,5),x(3,5)      
     $     ,v(2,5),x(2,5),v(1,5),x(1,5)
      if (time.lt.0) print 101,vh(3,5),xh(3,5)      
     $     ,vh(2,5),xh(2,5),vh(1,5),xh(1,5)
c      pause
      if (time.lt.0) print 101,v(3,5)-vh(3,5)
     $     ,x(3,5)-x(3,5)      
     $     ,v(2,5)-v(2,5),x(2,5)-xh(2,5)
     $     ,v(1,5)-vh(1,5),x(1,5)-xh(1,5)
...
c Local
      ...
      real*8 ha(2),hb(2),rt10,angf(3,NMAX),ausr(3,NMAX),vusr(3,NMAX)
c
c------------------------------------------------------------------------------
c
      rt10 = sqrt(10.d0)
...
c
        do j = 2, nbod
          ...
          xh(1,j) = xh(1,j) - hb(k) * vusr(1,j)
          xh(2,j) = xh(2,j) - hb(k) * vusr(2,j)
          xh(3,j) = xh(3,j) - hb(k) * vusr(3,j)
        end do
c
c Advance Keplerian Hamiltonian (Jacobi/helio coords for Big/Small bodies)
        call mco_h2j(time,jcen,nbig,nbig,h,m,xh,vh,xj,vj,ngf,ngflag,opt)
...
c
c If required, apply non-gravitational and user-defined forces
        if (opt(8).eq.1) call m
     $       fo_user (time,jcen,nbod,nbig,m,x,v,ausr,vusr)
...
c If required, apply non-gravitational and user-defined forces
        if (opt(8).eq.1)
     $       call mfo_user (time,jcen,nbod,nbig,m,x,v,ausr,vusr)
        if (ngflag.eq.1.or.ngflag.eq.3) call mfo_ngf (nbod,x,v,angf,ngf)
c
        do j = 2, nbod
          v(1,j) = v(1,j)  -  hb(k) * (angf(1,j) + ausr(1,j) + a(1,j))
...
c------------------------------------------------------------------------------
c
      if (time.lt.0) print *,'flag pre-uu2'
      if (time.lt.0) print 101,v(3,5),x(3,5)      
     $     ,v(2,5),x(2,5),v(1,5),x(1,5)
      if (time.lt.0) print 101,vh(3,5),xh(3,5)      
     $     ,vh(2,5),xh(2,5),vh(1,5),xh(1,5)
...
c
      ...
      if (time.lt.0) print 101,mvsum(1),mvsum(2),mvsum(3)
      if (time.lt.0) print *,'flag pre-uu3'
      if (time.lt.0) print 101,v(3,5),x(3,5)      
     $     ,v(2,5),x(2,5),v(1,5),x(1,5)
      if (time.lt.0) print 101,vh(3,5),xh(3,5)      
     $     ,vh(2,5),xh(2,5),vh(1,5),xh(1,5)
...
c
c Local
      integer j,nce,ice(NMAX),jce(NMAX),ce(NMAX),iflag,jj
      real*8 a(3,NMAX),hby2,hrec,x0(3,NMAX),v0(3,NMAX),mvsum(3),temp
      real*8 angf(3,NMAX),ausr(3,NMAX),vusr(3,NMAX)
      real*8 xhk(3,nmax),vhk(3,nmax)
      real*8 mbig(nbig),xbig(3,nbig),vbig(3,nbig),sbig(3,nbig)
      real*8 rphysbig(nbig),hrecbig
      integer nbigbig
      real*8 rcebig(nbig),rcritbig(nbig),ngfbig(4,nbig)
      external mfo_hkce
c
c------------------------------------------------------------------------------
c
      ...
      if (time.lt.0) print *,'flag 1'
      if (time.lt.0) print 100,time,v(3,5),x(3,5)
     $     ,v(2,5),x(2,5),v(1,5),x(1,5)
      hby2 = h0 * .5d0
      nclo = 0
      colflag = 0
cc      if (time.ge.333340000) print 100,time,v(3,5),x(3,5)
c     $     ,v(2,5),x(2,5),v(1,5),x(1,5)
c      if (mod(time,10000).eq.0.and.time.ge.333340000) pause

c If accelerations from previous call are not valid, calculate them now
c      if (dtflag.ne.2) then
c      if (time.lt.0) print *,'inhere'
        if (dtflag.eq.0) hrec = h0
        if (dtflag.eq.0) hrecbig = h0
...
c If required, apply non-gravitational and user-defined forces
c      if (time.lt.0) print *,'flag 2'
c      if (time.lt.0) print 100,v(3,5),x(3,5)
c     $     ,v(2,5),x(2,5),v(1,5),x(1,5)
        if (opt(8).eq.1) call
     $       mfo_user (time,jcen,nbod,nbig,m,x,v,ausr,vusr)
c      if (time.lt.0) print *,'flag 3'
c      if (time.lt.0)
c     $     print 100,time,v(3,5),x(3,5)      
c     $     ,v(2,5),x(2,5),v(1,5),x(1,5)
      if (ngflag.eq.1.or.ngflag.eq.3) call mfo_ngf (nbod,x,v,angf,ngf)
c      if (time.lt.0) print 100,v(3,5),x(3,5)
c     $     ,v(2,5),x(2,5),v(1,5),x(1,5)
c      if (time.lt.0) print *,'flag 4'
c Advance interaction Hamiltonian for H/2
      do j = 2, nbod
        ...
        x(1,j) = x(1,j)  +  hby2 * vusr(1,j)
        x(2,j) = x(2,j)  +  hby2 * vusr(2,j)
        x(3,j) = x(3,j)  +  hby2 * vusr(3,j)
      end do
c      if (time.lt.0) print 100,v(3,5),x(3,5)
c     $     ,v(2,5),x(2,5),v(1,5),x(1,5)
c      if (time.lt.0) print *,'flag 5'
c
...
c Advance H_K for H
      do j = 2, nbod
        iflag = 0
c      if (time.lt.0.and.j.eq.5) then
c         print 100,m(1),x(1,j),x(2,j),x(3,j)
c         print 100,h0,v(1,j),v(2,j),v(3,j)
c         print *,iflag
c      end if
        call drift_one (m(1),x(1,j),x(2,j),x(3,j),v(1,j),v(2,j),
     %    v(3,j),h0,iflag)
...
c      if (time.lt.0) print *,'flag 8'
c      if (time.lt.0) print 100,v(3,5),x(3,5)
c     $     ,v(2,5),x(2,5),v(1,5),x(1,5)
      call mco_iden (time,jcen,nbod,nbig,h0,m,x,v,
     $     xhk,vhk,ngf,ngflag,opt)
c      if (time.lt.0) print *,'flag 9'
c      if (time.lt.0) print 100,v(3,5),x(3,5)
c     $     ,v(2,5),x(2,5),v(1,5),x(1,5)
c
c Check whether any object separations were < R_CRIT whilst advancing H_K
      ...
      if (nce.gt.0) then
         do j=1,nce
            if ((ice(j).le.nbig.and.jce(j).le.nbig).and.j.ne.1) then
                        print *,j
                        print *,ice(j)
                        print *,jce(j)
c                        pause
...
c If objects had close encounters, advance H_K using Bulirsch-Stoer instead
     ...
     %    jxvclo,outfile,mem,lmem,mfo_hkce)
        ...
        do jj = 1,nce
c           if (time.lt.0) print *,nce
         do j = 1, nbig
           if ((ice(jj).gt.nbig.or.jce(jj).gt.nbig)) then
              x(1,j) = xhk(1,j)
              x(2,j) = xhk(2,j)
              x(3,j) = xhk(3,j)
              v(1,j) = vhk(1,j)
         ...
         if (ice(jj).le.nbig.and.jce(jj).le.nbig) then
          do j=1,nbig
            if (ce(j).ne.0) then
               xbig(1,j)=x0(1,j)
               xbig(2,j)=x0(2,j)
         ...
         nbigbig=nbig
        call mdt_hkce (time,tstart,h0,hrecbig,tol,rmax,en(3),jcen,rcen,
     %    nbigbig,nbig,mbig,xbig,vbig,sbig,rphysbig,rcritbig
     $       ,rcebig
     $        ,stat,id,ngf,algor,opt,
     %    ngflag,colflag,ce,nce,ice,jce,nclo,iclo,jclo,dclo,tclo,ixvclo,
     %    jxvclo,outfile,mem,lmem,mfo_hkce)
         do j=1,nbig
            if (ce(j).ne.0) then
...
c         if (time.lt.0) then
c         print *,nce
c         print 100,time,x0(1,5),xbig(1,5),xhk(1,5)
c         print 100,x(1,5),x(1,4),x(1,3),x(1,2)
c         print 100,v(1,5),v(1,4),v(1,3),v(1,2)
c         print *,ce(5),ce(4),ce(3),ce(2)
c         pause
c      end if
c      if (time.lt.0) print *,'flag 11'
c      if (time.lt.0) print 100,v(3,5),x(3,5)
c     $     ,v(2,5),x(2,5),v(1,5),x(1,5)  
...
c 3349398000
      if (time.lt.0) print *,'flag 12'
      if (time.lt.0) print 100,v(3,5),x(3,5)      
     $     ,v(2,5),x(2,5),v(1,5),x(1,5)
c      if (time.gt.334939800) pause
c
c------------------------------------------------------------------------------
c
      return
...
c Local
      integer j
      real*8 acor(3,NMAX),acen(3),vusr(3,NMAX)
c
c------------------------------------------------------------------------------
c
...
c Include user-defined accelerations if required
      if (opt(8).eq.1) then
        call mfo_user (time,jcen,nbod,nbig,m,x,v,acor,vusr)
        do j = 2, nbod
          a(1,j) = a(1,j) + acor(1,j)
          a(2,j) = a(2,j) + acor(2,j)
          a(3,j) = a(3,j) + acor(3,j)
          v(1,j) = v(1,j) + vusr(1,j)
          v(1,j) = v(1,j) + vusr(2,j)
          v(1,j) = v(1,j) + vusr(3,j)
        end do
      end if
c
c------------------------------------------------------------------------------
